...
### Phase 2 — open bugs & TODOs (field-tested 2026-07-13)

- ~~**Bug: session timer survives app dismissal from the task switcher.**~~ **Fixed 2026-07-14.** Confirmed root cause: swiping a watched app away in recents (RecentsActivity is deliberately treated as a transient overlay so glancing at recents doesn't kill a session) never fires a new, different foreground-change event — accessibility events and the usage-stats poll's own foreground tracking had nothing to react to. Fix: `DefangAccessibilityService.processUsageEventsSinceLastPoll` now watches for a `MOVE_TO_BACKGROUND` usage event matching `currentWatchedPackage` directly — task removal reliably emits this regardless of what (if anything) becomes foreground next — and ends the session immediately on it, independent of the foreground-change path. Verified on device across three full pass-gate → leave (task-switcher swipe or otherwise) → reopen cycles: session ended and the gate correctly reappeared every time.
- ~~**Bug: Snapchat opens in color with no intent gate.**~~ **Fixed 2026-07-14.** Root cause: Snapchat's window never delivers `TYPE_WINDOW_STATE_CHANGED` to the accessibility service at all — confirmed via `DefangSvc` logcat trace across multiple real opens, zero `windowState pkg=com.snapchat.android` lines, window flags otherwise unremarkable (no `FLAG_SECURE`). Likely an anti-accessibility-service measure on Snapchat's part rather than a Defang decision-path bug. Fix: added a `UsageStatsManager`-based foreground-poll fallback (`DefangAccessibilityService.pollForegroundAppLoop`, 2s cadence) that feeds detected foreground changes through the same `handleForegroundChange` path as real accessibility events — verified the gate now fires for Snapchat on a real device. Trade-off: up to ~2s latency vs. the near-instant accessibility-event path, only for apps that evade the event entirely. Separately found and fixed a real bug in `AccessibilityServiceHelper.ensureEnabled` that silently claimed success without verifying the accessibility service actually rebound after a reinstall.
- ~~**Bug: Snapchat opens with no intent gate again (regression).**~~ **Fixed 2026-08-17.** The 2026-07-14 `UsageStatsManager` poll fallback stopped working: `queryEvents()` no longer returns any events for `com.snapchat.android` to third-party callers, confirmed via side-by-side comparison with `adb shell dumpsys usagestats`, which — running with system privilege — still recorded Snapchat's opens/closes normally in the same window our poll saw nothing. Ruled out first: Usage Access permission (granted), battery optimization (set to unrestricted), `QUERY_ALL_PACKAGES` (granted), Samsung "Usage data access" and App Lock (neither applies), Secure Folder (not in use). This is the second distinct detection channel Snapchat has evaded — first `TYPE_WINDOW_STATE_CHANGED` (2026-07-14), now `UsageStatsManager` visibility itself — while every other watched app (Instagram, Bumble, TikTok, etc.) continues to report normally through both. Fix: added `DefangAccessibilityService.checkEvasiveAppWindows`, a `getWindows()`-based poll (same 2s cadence, `EVASIVE_WINDOW_ONLY_PACKAGES` set) that reads the live window list the OS itself maintains for the accessibility layer rather than a self-reported event or a queryable log — a channel Snapchat cannot decline to appear in without breaking its own on-screen rendering. Scoped to the small evasive-app set rather than running for everyone, since the existing two paths remain cheaper and lower-latency for apps that don't actively resist being observed.
- ~~**TODO: gate on every open — drop the re-gate grace across apps.**~~ **Fixed 2026-07-14.** Removed `lastGatePassedMs`/`REGATE_GRACE_MS`/`withinRegateGrace` entirely from `DefangAccessibilityService` — there is no more time-based silent bypass anywhere. Same-app internal navigation is still tolerated (the pre-existing `pkg == currentWatchedPackage` in-session check handles that on its own, no grace needed). Verified on device: pass the gate in Snapchat, switch to a different app, return to Snapchat — gate reappears every time.
  - **Surfaced while testing**: swiping a watched app away in the task switcher didn't reliably end its session (same root cause as the task-switcher bug above) — meaning a reopen could still skip the gate under the "session active" rule. Fixed together with that bug on 2026-07-14; re-gate now holds after a task-switcher close too.
- ~~**Design constraint: lock screen must never show color.**~~ **Fixed 2026-07-14.** Root cause: `grayscale.disable()` was unconditional at every call site (e.g. any unwatched-app foreground event), so any accessibility event firing while the phone was locked but before `ACTION_USER_PRESENT` — a fingerprint prompt, a stray system window, a missed `SCREEN_OFF` broadcast on service restart — could flip color back on under the lock screen. Fix: `GrayscaleController.isDeviceLocked()` (via `KeyguardManager`) is now the single source of truth, checked inside `disable()` itself so no call site can restore color while locked; `enable()` is additionally re-asserted at service (re)connect and on every 2s poll tick, so gray no longer depends on having witnessed a `SCREEN_OFF` broadcast at all. Verified on device: locked, glanced at notifications, unlocked — grayscale held throughout, color returned only on actual unlock.
