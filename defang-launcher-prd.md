...
- **Bug: session timer survives app dismissal from the task switcher.** Swiping the watched app away in recents does not end the session — the timer keeps counting with no app open. Likely related to recents events being ignored (RecentsActivity is treated as a transient overlay so it doesn't kill sessions); dismissing the app from recents therefore produces no foreground-change event that would end the session. The session should end (or pause) when the watched app's task is killed.
- **Bug: Snapchat opens in color with no intent gate.** com.snapchat.android is on the default watched list, but the gate does not fire and grayscale is not applied. Needs a logcat trace (tag `DefangSvc`) to see whether the window event arrives and where the decision path bails.
- **TODO: gate on every open — drop the re-gate grace across apps.** Apps opened via the trace-to-unlock can currently be re-entered for up to 5 minutes (`REGATE_GRACE_MS`) without a new trace. Switching Snapchat → Instagram must demand a fresh trace even within the grace window. Every watched-app open should require the trace unlock, regardless of time remaining. (The grace exists because in-app navigation fires TYPE_WINDOW_STATE_CHANGED for the same package — any fix must still tolerate same-app internal window changes, but must never carry the grace across to a different package, and same-app re-entry after actually leaving the app should also re-gate.)
- **Design constraint: lock screen must never show color.** The lock screen currently flips between color and grayscale depending on prior state instead of always rendering colorless. This matters beyond visual polish — a colorful lock screen is a cue in the cue-routine-reward habit loop the whole app is designed to interrupt. Any cue at all (even briefly, even just at glance-to-check-notifications) re-primes the habit loop before the user has consciously decided to open anything. The lock screen should be unconditionally grayscale, independent of session/gate state, not just "gray by default unless something restored color."
